...
public class SecondServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// get access to existing session object of the current browser s/w
		HttpSession session = request.getSession(false); // It will use exisiting session

		// Read from1/request1 data from http session objects as session attributes
		// values
		pname = (String) session.getAttribute("pname");
		pfname = (String) session.getAttribute("pfname");
		paddress = (String) session.getAttribute("paddress");
		...
		pw.println("<p style='color: green; text-align: center'>" + pname + " " + pfname + " " + paddress + " " + ms
				+ "</p> <br>");
		...
		pw.print("<br><br><h2 style='color:red;text-align:center'>SessionId: "+ session.getId() +"</h2>");
